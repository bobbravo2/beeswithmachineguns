...
<pre>
bees up -s 4 -g public -k frakkingtoasters -z us-east-#
bees attack -n 10000 -c 250 -u http://www.ournewwebbyhotness.com/
bees down
</pre>

h3. bees up -parameters
...
* The -z specifies the EC2 zone to deploy to
...
h3. bees attack -parameters

* *n* total number of connections/requests to make
* *c* concurrency level of connections
* *u* url to test 

*NOTES*
* the n & c parameters are *divided* among your swarm. For those familiar with running apacheBench that means: ab -n (parameter / # bees)
* Always include a trailing slash when testing a root domain. The underlying load-testing tool (ab) doesn't support raw domains.
